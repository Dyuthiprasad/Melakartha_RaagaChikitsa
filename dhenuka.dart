...
class DhenukaScreen extends StatefulWidget {
  const DhenukaScreen({super.key});

  @override
  State<DhenukaScreen> createState() => _DhenukaScreenState();
}

class _DhenukaScreenState extends State<DhenukaScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M9/M9($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M9($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
