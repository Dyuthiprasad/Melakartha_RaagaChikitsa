...
class ManavatiScreen extends StatefulWidget {
  const ManavatiScreen({super.key});

  @override
  State<ManavatiScreen> createState() => _ManavatiScreenState();
}

class _ManavatiScreenState extends State<ManavatiScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M5/M5($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M5($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBenefitRow('Fosters a sense of compassion and empathy towards others.'),
              _buildBenefitRow('Promotes inner warmth and a feeling of contentment.'),
              _buildBenefitRow('Cultivates patience and understanding in interactions.'),
              _buildBenefitRow('May help in dissolving emotional blockages and resentment.'),
              const SizedBox(height: 16),
              Text('Physical Health Benefits:', style: TextStyle(color: brown, fontSize: screenWidth * 0.05)),
              const SizedBox(height: 8),
              _buildBenefitRow('Potentially soothes the nervous system, leading to profound relaxation.'),
              _buildBenefitRow('May ease physical manifestations of emotional stress, such as muscle tension.'),
              _buildBenefitRow('Supports healthy digestive function by promoting a calm internal state.'),
              _buildBenefitRow('Contributes to overall vitality by reducing mental and emotional burdens.'),
            ],
          ),
        ),
      ),
