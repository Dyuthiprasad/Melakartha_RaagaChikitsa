...
class NatakapriyaScreen extends StatefulWidget {
  const NatakapriyaScreen({super.key});

  @override
  State<NatakapriyaScreen> createState() => _NatakapriyaScreenState();
}

class _NatakapriyaScreenState extends State<NatakapriyaScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M10/M10($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M10($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBenefitRow('Fosters a sense of comfort and emotional warmth.'),
              _buildBenefitRow('Encourages a gentle and agreeable disposition.'),
              _buildBenefitRow('Promotes emotional balance and inner harmony.'),
              _buildBenefitRow('Supports a lighthearted and optimistic outlook.'),
              const SizedBox(height: 16),
              Text('Physical Health Benefits:', style: TextStyle(color: brown, fontSize: screenWidth * 0.05)),
              const SizedBox(height: 8),
              _buildBenefitRow('Soothes the nervous system, inducing a gentle state of relaxation.'),
              _buildBenefitRow('Helps in easing physical tension associated with daily stress.'),
              _buildBenefitRow('Supports rhythmic and calm breathing patterns, beneficial for lung capacity.'),
              _buildBenefitRow('Indirectly aids in minor digestive discomforts by promoting a relaxed state.'),
            ],
          ),
        ),
      ),
