...
class SenavatiScreen extends StatefulWidget {
  const SenavatiScreen({super.key});

  @override
  State<SenavatiScreen> createState() => _SenavatiScreenState();
}

class _SenavatiScreenState extends State<SenavatiScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M7/M7($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M7($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBenefitRow('Sharpens intellect and promotes mental alertness.'),
              _buildBenefitRow('Encourages clear communication and articulate expression.'),
              _buildBenefitRow('Fosters a sense of determination and resolute focus.'),
              _buildBenefitRow('Supports analytical thinking and precise understanding.'),
              const SizedBox(height: 16),
              Text('Physical Health Benefits:', style: TextStyle(color: brown, fontSize: screenWidth * 0.05)),
              const SizedBox(height: 8),
              _buildBenefitRow('Stimulates vital energy, helping to combat lethargy.'),
              _buildBenefitRow('Promotes healthy blood circulation, invigorating the system.'),
              _buildBenefitRow('Aids in balancing the digestive fire and improving metabolism.'),
              _buildBenefitRow('Supports optimal respiratory function through focused breath control during singing.'),
            ],
          ),
        ),
      ),
