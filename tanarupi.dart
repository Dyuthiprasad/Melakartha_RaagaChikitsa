...
class TanarupiScreen extends StatefulWidget {
  const TanarupiScreen({super.key});

  @override
  State<TanarupiScreen> createState() => _TanarupiScreenState();
}

class _TanarupiScreenState extends State<TanarupiScreen> {
  ...
  Future<void> setAudioByIndex(int index) async {
    try {
      await audioPlayer.setSource(AssetSource('audio/M6/M6($index).wav'));
      if (_mounted) {
        setState(() {
          isAudioLoaded = true;
        });
      ...
      print('Error loading audio asset M6($index).wav: $e');
      if (_mounted) {
        setState(() {
          isAudioLoaded = false;
        });
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _buildBenefitRow('Fosters profound inner peace and a sense of calm serenity.'),
              _buildBenefitRow('Encourages introspection and deep meditation.'),
              _buildBenefitRow('Supports emotional detachment from worldly worries, promoting mental clarity.'),
              _buildBenefitRow('Cultivates a sense of detachment from superficial thoughts, enhancing focus.'),
              const SizedBox(height: 16),
              Text('Physical Health Benefits:', style: TextStyle(color: brown, fontSize: screenWidth * 0.05)),
              const SizedBox(height: 8),
              _buildBenefitRow('Gently lowers stress and anxiety levels, soothing the entire system.'),
              _buildBenefitRow('Supports a slow and steady breathing rhythm, benefiting respiratory health.'),
              _buildBenefitRow('May aid in regulating blood pressure by inducing a state of deep relaxation.'),
              _buildBenefitRow('Contributes to restful sleep by calming an overactive mind.'),
            ],
          ),
        ),
      ),
